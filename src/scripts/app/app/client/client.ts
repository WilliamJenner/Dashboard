...
export interface IClient {
  ...
  cat_RandomCatUrl(): Promise<string>;
  image_RandomImage(): Promise<string>;
...
export class Client implements IClient {
  ...
  cat_RandomCatUrl(): Promise<string> {
    ...
    return this.http.fetch(url_, options_).then((_response: Response) => {
      return this.processCat_RandomCatUrl(_response);
    });
  }

  protected processCat_RandomCatUrl(response: Response): Promise<string> {
    const status = response.status;
    let _headers: any = {};
    if (response.headers && response.headers.forEach) {
      response.headers.forEach((v: any, k: any) => (_headers[k] = v));
  ...
  image_RandomImage(): Promise<string> {
    ...
    return this.http.fetch(url_, options_).then((_response: Response) => {
      return this.processImage_RandomImage(_response);
    });
  }

  protected processImage_RandomImage(response: Response): Promise<string> {
    const status = response.status;
    let _headers: any = {};
    if (response.headers && response.headers.forEach) {
      response.headers.forEach((v: any, k: any) => (_headers[k] = v));
  ...
  protected processStatus_GetStats(response: Response): Promise<Status[]> {
    ...
    if (status === 500) {
      ...
      return response.text().then((_responseText) => {
        ...
        if (Array.isArray(resultData200)) {
          result200 = [] as any;
          for (let item of resultData200) result200!.push(Status.fromJS(item));
        }
        return result200;
      });
    } else if (status !== 200 && status !== 204) {
  ...
  protected processStatus_GetProcessInfo(
    response: Response
  ): Promise<ProcessInfoResult[]> {
    const status = response.status;
    let _headers: any = {};
    if (response.headers && response.headers.forEach) {
      response.headers.forEach((v: any, k: any) => (_headers[k] = v));
  ...
  terraria_GetLogsTail(secondsToWait: number): Promise<string[]> {
    ...
    url_ = url_.replace(
      "{secondsToWait}",
      encodeURIComponent("" + secondsToWait)
    );
    url_ = url_.replace(/[?&]$/, "");
  ...
  init(_data?: any) {
    if (_data) {
      ...
      this.publishedAt = _data["publishedAt"]
        ? new Date(_data["publishedAt"].toString())
        : <any>undefined;
    }
  }
...
export class WallboardInfo implements IWallboardInfo {
  ...
  toJSON(data?: any) {
    ...
    if (Array.isArray(this.processes)) {
      data["processes"] = [];
      for (let item of this.processes) data["processes"].push(item.toJSON());
    }
    return data;
  }
}
